...
(() => {
    ...
    const VERSION = typeof GM_info !== "undefined" && GM_info?.script?.version ? GM_info.script.version : "1.3.42-beta.7";
    ...
    function renderSettings() {
        ...
        const saveBackupLabel = state.runtimeKind === "tornpda" ? "Save to TornPDA Downloads" : "Save backup file";
        const backupRestore = section("Backup & restore", `<label class="ncc-check"><input id="ncc-backup-include-keys" type="checkbox"><span><b>Include saved Director keys in this backup</b><br>Unchecked by default. Keys are never displayed, logged, or included unless selected for this single download.</span></label><div class="ncc-inline" style="margin-top:10px"><button class="ncc-button ncc-primary" data-action="save-company-backup">${saveBackupLabel}</button><button class="ncc-button" data-action="share-company-backup">Share backup…</button><button class="ncc-button" data-action="choose-company-backup">Restore from device or Drive…</button><input id="ncc-company-backup-file" type="file" accept=".json,application/json,text/json,text/plain" style="position:absolute;width:1px;height:1px;opacity:0;pointer-events:none"></div><p class="ncc-note">Desktop browsers use their native Save As picker when available. TornPDA does not expose Android/iOS document-save picker access to userscripts, so Save writes once to TornPDA’s app Downloads folder; TornPDA’s native download toast reports the exact path. Set <b>TornPDA Settings → Browser → Downloads → Download action</b> to <b>Save</b>. Share opens Android/iOS’s share sheet, whose destinations are controlled by installed apps—Samsung My Files does not register as a share target. Restore does use the system document picker, where local storage, Google Drive, iCloud Drive, and other installed document providers can be selected. Backups include separate company snapshots, history, rankings, planner data, layout, settings, daily-sync state, and alerts. API keys stay out unless you opt in both when creating and restoring a key-containing backup.</p>`);
        ...
        try {
            ...
            state.status = destination === "share" && result.shared
                ...
                : result.transport === "picker"
                    ...
                    : result.runtime === "tornpda"
                        ? "Company backup sent to TornPDA’s app Downloads folder; its native toast shows the exact path " + detail
