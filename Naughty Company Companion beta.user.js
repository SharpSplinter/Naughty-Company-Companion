...
(() => {
    ...
    const VERSION = typeof GM_info !== "undefined" && GM_info?.script?.version ? GM_info.script.version : "1.3.42-beta.8";
    ...
    function renderSettings() {
        ...
        const saveBackupLabel = state.runtimeKind === "tornpda" ? "Save or share backup…" : "Save backup file";
        const backupRestore = section("Backup & restore", `<label class="ncc-check"><input id="ncc-backup-include-keys" type="checkbox"><span><b>Include saved Director keys in this backup</b><br>Unchecked by default. Keys are never displayed, logged, or included unless selected for this single export.</span></label><div class="ncc-inline" style="margin-top:10px"><button class="ncc-button ncc-primary" data-action="save-company-backup">${saveBackupLabel}</button><button class="ncc-button" data-action="choose-company-backup">Restore from device or Drive…</button><input id="ncc-company-backup-file" type="file" accept=".json,application/json,text/json,text/plain" style="position:absolute;width:1px;height:1px;opacity:0;pointer-events:none"></div><p class="ncc-note">On TornPDA, Save uses the documented native <code>shareFile({ base64Data, fileName })</code> handler and opens Android/iOS’s save-or-share sheet. Destinations in that sheet are controlled by installed apps; Samsung My Files does not register as an Android share target, so TornPDA or a userscript cannot add it there. A true folder picker would require TornPDA to expose a native Android <code>ACTION_CREATE_DOCUMENT</code> handler. Desktop browsers use their native Save As picker when available. Restore does use the system document picker, where local storage, Google Drive, iCloud Drive, and other installed document providers can be selected. Backups include separate company snapshots, history, rankings, planner data, layout, settings, daily-sync state, and alerts. API keys stay out unless you opt in both when creating and restoring a key-containing backup.</p>`);
