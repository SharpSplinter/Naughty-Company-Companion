...
The daily-tick assistant runs while the Company page/userscript remains active. It can produce an 18:00 UTC alert with full Daily Income, Daily Profit, Daily Customer Count, Star Level, and integer Stock Difference versus the prior day, then an employee addiction/inactivity-risk alert at 18:10 UTC. Daily-tick toasts and notifications are separate opt-in Settings choices and are disabled by default; disabling notifications also removes TornPDA’s scheduled daily reminders. When the current snapshot predates that tick, it silently refreshes first; otherwise the message labels the data as cached or unavailable. Enabled TornPDA delivery uses native toast and notification handlers, and the companion keeps detailed in-panel toast cards stacked so one alert does not replace another. Desktop uses the userscript/browser-notification fallback. User-triggered save/refresh/export feedback also prefers the native toast handler with a desktop in-panel fallback.

When daily-tick notifications are enabled, confirmed TornPDA installs also keep one generic native background reminder for each next phase (18:00 and 18:10 UTC), with a callback to the Company page. These reminders never claim live figures: they tell you to open the companion and refresh. When the Company page is active, the script cancels that phase's generic reminder immediately before the tick, sends the detailed live alert, and establishes the following day's generic reminder. The schedules are refreshed on native startup/confirmation and removed when notifications are disabled or local companion data is cleared. Desktop deliberately has no simulated background-reminder capability. Scheduled page work and automatic core refreshes pause while the document is hidden, then timers and due-day checks resume safely when it becomes visible.
