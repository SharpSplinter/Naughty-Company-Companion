...
(() => {
    ...
    const DEFAULT_LAYOUT = { x: null, y: 18, width: 940, height: 660, minimized: false };
    const state = {
            ...
            <section id="ncc-panel" aria-label="Naughty Company Companion">
                <header class="ncc-head" id="ncc-drag-handle">
                    ...
                    <div class="ncc-head-actions">
                        <button class="ncc-icon" type="button" data-action="refresh" title="Refresh company data">↻</button>
    ...
    function renderOverview() {
        ...
        return `${dataNotice()}${grid}${section("Income rank & star outlook", rankBody, `<button class="ncc-button ncc-primary" data-action="load-rankings" ${state.rankingLoading ? "disabled" : ""}>${state.rankingLoading ? "Loading…" : rankings ? "Refresh rankings" : "Load rankings"}</button>`)}<div class="ncc-grid ncc-grid-2">${section("Company condition", conditionBody)}${section("Recent funds news", recentNews)}</div><p class="ncc-note">Revenue is supplied by Torn. Net is an estimate: daily revenue − sold stock cost − ads − wages; weekly net excludes stock cost because Torn exposes sold stock as a daily value. Monthly values become tracked totals after enough local daily snapshots.</p>`;
    }
    ...
    function renderTeam() {
        ...
        const table = rows.length ? `<div class="ncc-table-wrap"><table class="ncc-table"><thead><tr><th>Lock</th>${sortHeader("Employee", "name", "team")}${sortHeader("Current position", "position", "team")}${sortHeader("Current eff.", "total", "team")}${sortHeader("Assigned position", "assignedPosition", "team")}${sortHeader("Assigned eff.", "assignedEfficiency", "team")}${sortHeader("Addiction eff.", "addiction", "team")}${sortHeader("Inactivity eff.", "inactivity", "team")}${sortHeader("Best fit", "best", "team")}${sortHeader("Wage", "wage", "team")}<th>Last action</th></tr></thead><tbody>${rows.map((row) => {
            ...
            return `<tr class="${misplaced ? "ncc-misplaced" : ""}"><td><input type="checkbox" data-lock-employee="${row.id}" ${row.locked ? "checked" : ""} title="Keep this employee in their current position when auto-assigning"></td><td><b>${escapeHtml(row.name)}</b><br><span class="ncc-muted">${escapeHtml(row.status)} · ${formatOptionalNumber(row.days)}d</span></td><td>${escapeHtml(row.currentPosition || "—")}<br><span class="ncc-muted">Base ${formatOptionalNumber(row.projectedCurrent, 1)}</span></td><td><b>${formatOptionalNumber(row.currentEfficiency, 1)}</b><br><span class="ncc-muted">Work stats ${formatOptionalNumber(row.workingStats, 1)}</span></td><td><select class="ncc-select" data-assignment="${row.id}" ${positions.length ? "" : "disabled"}>${selectOptions || "<option>Load projections</option>"}</select></td><td class="${misplaced ? "ncc-warn" : ""}"><b>${formatOptionalNumber(row.assignedEfficiency, 1)}</b><br><span class="ncc-muted">Δ ${formatSignedNumber(row.nonWorkingDelta)}</span></td><td class="${asNumber(row.addiction) < 0 ? "ncc-bad" : ""}">${formatSignedNumber(row.addiction)}</td><td class="${asNumber(row.inactivity) < 0 ? "ncc-bad" : ""}">${formatSignedNumber(row.inactivity)}</td><td>${escapeHtml(row.bestPosition || "—")}<br><span class="ncc-good">${formatOptionalNumber(row.bestEfficiency, 1)}</span></td><td>${formatMoney(row.wage, true)}</td><td>${escapeHtml(lastAction)}</td></tr>`;
        }).join("")}</tbody></table></div>` : `<div class="ncc-notice">No employee rows match the filter, or employee details are not available for this API key.</div>`;
        ...
        return `${dataNotice()}<div class="ncc-toolbar"><input class="ncc-input" id="ncc-team-filter" type="search" value="${escapeHtml(state.teamFilter)}" placeholder="Filter employee or role"><button class="ncc-button ncc-primary" data-action="load-projections" ${state.projectionLoading ? "disabled" : ""}>${state.projectionLoading ? "Loading…" : "Load efficiency projections"}</button><button class="ncc-button" data-tab="planner">Open planner</button><span class="ncc-help">Actual efficiency comes from Torn. Projections are optional.</span></div>${section("Employee efficiency", table)}<div class="ncc-grid ncc-grid-3">${metricCard("Employees", formatNumber(rows.length), `${formatNumber(state.data?.profile?.employees?.capacity)} capacity`)}${metricCard("Avg. current efficiency", values.length ? formatNumber(values.reduce((sum, value) => sum + asNumber(value), 0) / values.length, 1) : "—", "Torn reported total")}${metricCard("Efficiency penalties", formatNumber(affected), "Addiction or inactivity effects", affected ? "ncc-warn" : "ncc-good")}</div><p class="ncc-note">Assigned efficiency = TornStats’ education-assumed position projection + the employee’s current non-working-stat effect delta. This is a planning estimate; it does not change Torn assignments.</p>`;
    }
    ...
    function renderPlanner() {
        ...
        return `${dataNotice()}<div class="ncc-toolbar"><button class="ncc-button" data-action="save-planner" ${positions.length ? "" : "disabled"}>Save capacity settings</button><button class="ncc-button ncc-primary" data-action="auto-assign" ${positions.length ? "" : "disabled"}>Auto-assign unlocked staff</button><button class="ncc-button" data-action="load-projections" ${state.projectionLoading ? "disabled" : ""}>${state.projectionLoading ? "Loading…" : "Load / refresh projections"}</button><span class="ncc-help">0 or blank max qty = uncapped. Lower priority number fills first.</span></div>${section("Position capacity & priority", capacityRows)}${warnings ? `<div class="ncc-notice warn">${warnings}</div>` : ""}${section("Assignment preview", rowsTable)}<p class="ncc-note">Auto assignment keeps locked employees in their current seats, then greedily fills positions by priority with the highest remaining base efficiency while respecting configured role caps and total company capacity. It only saves a local plan.</p>`;
    }
    ...
    function renderRankings() {
        ...
        return `${dataNotice()}<div class="ncc-toolbar"><input class="ncc-input" id="ncc-rankings-filter" type="search" value="${escapeHtml(state.rankingsFilter)}" placeholder="Filter company or rank"><button class="ncc-button ncc-primary" data-action="load-rankings" ${state.rankingLoading ? "disabled" : ""}>${state.rankingLoading ? "Loading…" : "Refresh rankings"}</button><button class="ncc-button" data-action="show-health">View rank neighbors</button><span class="ncc-help">Updated ${timeAgo(record.fetchedAt)} · ${formatNumber(metrics.total)} companies</span></div>${section("Your company", `${strip}<p class="ncc-note">Health score is your weekly-income percentile among the same company type. Star slots use a ${slotSource}; the earnings gaps are observed projections, not official Torn thresholds.</p>`)}${section("Same-type companies", `${table}${rows.length > shown.length ? `<p class="ncc-note">Showing the first ${formatNumber(shown.length)} filtered companies.</p>` : ""}`)}`;
    }
    ...
    function renderStock() {
        ...
        rows = sortRows(rows, { key: (item) => sort.key === "margin" ? asNumber(item.sold_worth) - asNumber(item.cost) * asNumber(item.sold_amount) : item[sort.key], dir: sort.dir });
        const table = rows.length ? `<div class="ncc-table-wrap"><table class="ncc-table"><thead><tr>${sortHeader("Item", "name", "stock")}${sortHeader("In stock", "in_stock", "stock")}${sortHeader("On order", "on_order", "stock")}${sortHeader("Cost", "cost", "stock")}${sortHeader("Price", "price", "stock")}${sortHeader("Sold", "sold_amount", "stock")}${sortHeader("Sold worth", "sold_worth", "stock")}${sortHeader("Gross margin", "margin", "stock")}</tr></thead><tbody>${rows.map((item) => {
            const margin = asNumber(item.sold_worth) - asNumber(item.cost) * asNumber(item.sold_amount);
            return `<tr><td><b>${escapeHtml(item.name || "Unknown")}</b><br><span class="ncc-muted">ID ${formatNumber(item.id)}</span></td><td>${formatNumber(item.in_stock)}</td><td>${formatNumber(item.on_order)}</td><td>${formatMoney(item.cost)}</td><td>${formatMoney(item.price)}</td><td>${formatNumber(item.sold_amount)}</td><td>${formatMoney(item.sold_worth)}</td><td class="${margin >= 0 ? "ncc-good" : "ncc-bad"}">${formatMoney(margin)}</td></tr>`;
        }).join("")}</tbody></table></div>` : `<div class="ncc-notice warn">Stock details require a Limited or higher Torn API key.</div>`;
        return `${dataNotice()}<div class="ncc-grid ncc-grid-3">${metricCard("Stock items", formatNumber(totals.inStock), `${formatNumber(totals.onOrder)} on order`)}${metricCard("Stock value", formatMoney(totals.saleValue), `${formatMoney(totals.costValue)} at cost`, "ncc-good")}${metricCard("Reported gross margin", formatMoney(totals.margin), `${formatMoney(totals.soldWorth)} sold worth`, totals.margin >= 0 ? "ncc-good" : "ncc-bad")}</div>${section("Stock & sales", table)}<p class="ncc-note">Reported gross margin = sold worth − (cost × sold amount). Torn’s stock fields are used in the daily-net estimate only when “include sold stock cost” is enabled in Settings.</p>`;
    }
    ...
    const testApi = { reportingPeriod, weekKey, countStars, calculateRankingMetrics, financials, statFingerprint, projectionBlock, assignProjectedRows };
